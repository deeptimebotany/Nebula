...
/**
 * Données préparées par le serveur pour le premier affichage (lot 10, voir
 * page.tsx). Utilisées seulement si la marque active est toujours la même.
 */
export interface DashboardInitial {
  brandId: string;
  posts: ApiPost[];
  insights: InsightsResponse;
  linkPagePublished: boolean;
  reussites: ReussitesSummaryDTO | null;
  /** Variante du message d'accueil, tirée par le serveur (même texte des deux côtés). */
  greetingIndex: number;
}

// Cosmétique "Message d'accueil personnalisé" : variantes dans
// src/lib/dashboard-greetings.ts (partagées avec page.tsx, qui tire la
...
export function DashboardClient({ initial }: { initial: DashboardInitial | null }) {
  ...
  // Comptes et statistiques : cache partagé avec l'en-tête, Analytics et
  // Comptes (lot 6) — affichés tout de suite en revenant sur la page.
  const connectionsData = useConnections<ConnectionRow>(activeBrand?.id);
  const analyticsData = useAnalytics<AnalyticsConnection>(activeBrand?.id);
  const connections = useMemo(() => connectionsData.connections ?? [], [connectionsData.connections]);
  const analyticsConnections = useMemo(() => analyticsData.connections ?? [], [analyticsData.connections]);
  // Premier affichage : données du serveur (lot 10) — plus d'appels à
  // attendre ; ensuite (autre marque), chargement comme avant.
  const [posts, setPosts] = useState<ApiPost[]>(initial?.posts ?? []);
  const [postsLoading, setPostsLoading] = useState(!initial);
  const loading =
    postsLoading ||
    (connectionsData.connections === null && !connectionsData.error) ||
    (analyticsData.connections === null && !analyticsData.error);
  const [insights, setInsights] = useState<InsightsResponse | null>(initial?.insights ?? null);
  const [insightsFailed, setInsightsFailed] = useState(false);
  const [linkPagePublished, setLinkPagePublished] = useState<boolean | null>(initial?.linkPagePublished ?? null);
  ...
  useEffect(() => {
    ...
    // Données du serveur pour cette marque, tout juste préparées : rien à recharger.
    if (seededBrand && seedFresh) return;
    // Données déjà affichées mais rejouées : mise à jour discrète, sans
    // squelette. Autre marque : chargement comme avant.
    if (!seededBrand) setPostsLoading(true);
    ...
    const safe = (url: string): Promise<Loose> =>
      fetch(url)
        ...
        .catch(() => ({}));
    // Publications : version allégée, sur les deux dernières années (la
    // traînée « Momentum » remonte au plus 104 semaines) — audit
    // performance, lot 4.
    const since = new Date(Date.now() - 105 * 7 * 86_400_000).toISOString();
    safe(`/api/posts?brandId=${activeBrand.id}&view=light&from=${encodeURIComponent(since)}`)
      .then((postsData) => setPosts((postsData.posts as ApiPost[] | undefined) ?? []))
  ...
  // Début du mois à l'heure du navigateur : calculé après l'hydratation
  // (le serveur est en UTC, voir use-hydrated.ts).
  const startOfMonth = new Date();
  startOfMonth.setDate(1);
  startOfMonth.setHours(0, 0, 0, 0);
  const postsThisMonth = hydrated ? posts.filter((p) => new Date(p.createdAt) >= startOfMonth).length : null;
  ...
  return (
    <div className="space-y-8">
      ...
      <RevealGroup className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <RevealItem>
          <StatCard label="Posts ce mois-ci" value={postsThisMonth === null ? "—" : String(postsThisMonth)} icon={<IconCalendar className="h-4 w-4" />} />
        </RevealItem>
      </RevealGroup>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Reveal delay={0.05}>
          <MotionGlassCard className="h-full">
            ...
            {hydrated ? <MomentumComet streak={streak} /> : <SkeletonText lines={2} />}
          </MotionGlassCard>
        </Reveal>
                  ...
                  <div>
                    ...
                    {n.hasEnoughData && n.bestHour !== null ? (
                      <p className="mt-2 text-sm text-slate-300">
                        Prochain créneau :<br />
                        <span className="font-display text-base text-white">{hydrated ? nextOccurrence(n.bestHour) : `à ${n.bestHour}h`}</span>
                      </p>
                    ) : (
